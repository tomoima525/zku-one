...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * (Fr_toInt(&lvar[2]) + 1)) + 1)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
MiMCSponge_1_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &lvar[3];
// load src
Fr_div(&expaux[1],&lvar[2],&circuitConstants[5]); // line circom 72
Fr_add(&expaux[0],&expaux[1],&circuitConstants[9]); // line circom 72
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
